...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
  ...
  /* SDL_SetAppMetadata("Example Camera Read and Draw", "1.0",
...
*/
  ...
  if (!SDL_CreateWindowAndRenderer("examples/camera/read-and-draw", 640, 480, 0,
                                   &window, &renderer)) {
  ...
  if (devices == NULL) {
    ...
    SDL_Log("Couldn't find any camera devices! Please connect a camera and try "
            "again.");
    return SDL_APP_FAILURE;
  }

  camera = SDL_OpenCamera(
      devices[0],
      NULL); // just take the first thing we see in any format it wants.
  SDL_free(devices);
  if (camera == NULL) {
...
SDL_AppResult SDL_AppIterate(void *appstate) {
  ...
  if (frame != NULL) {
    ...
    if (!texture) {
      SDL_SetWindowSize(window, frame->w,
                        frame->h); /* Resize the window to match */
      texture =
          SDL_CreateTexture(renderer, frame->format,
